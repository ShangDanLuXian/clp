 ...
 * `CLP_TELEMETRY_CONSOLE_EXPORTER`). Matches the truthy values recognized by the CLP package
 * (`_TELEMETRY_DISABLE_VALUES` in `start_clp.py`).
 */
...
auto get_trimmed_lowercase_env(char const* name) -> std::string {
    ...
    auto const not_space{[](unsigned char c) {
        return false == static_cast<bool>(std::isspace(c));
    }};
